...
class HrOverTime(models.Model):
    ...
    def submit_to_f(self):
        ...
        msg = _(body)
        return self.sudo().write({"state": "f_approve"})
        ...
        for req in self:
            ...
            if nholidays:
                raise ValidationError(
                    _("You can not have 2 Overtime requests that overlaps on same day!")
                )
...
class HrOverTimeType(models.Model):
    ...
    leave_type = fields.Many2one(
        "hr.leave.type", string="Leave Type", domain="[('id', 'in', leave_compute)]"
    )
